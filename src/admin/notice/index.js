/*CSS*/
/* global PatternsConstructionLocalize */
...
class PatternsConstructionSettingsApi {
	...
	bindEvents() {
		document.addEventListener( 'click', ( event ) => {
			...
			if ( target.closest( '.patterns-construction-gsn-close' ) ) {
				this.modifySettings( 'hide_get_started_notice', true );
			} else if (
				target.closest( '.patterns-construction-rn-permanent-close' )
			) {
				this.modifySettings( 'remove_review_notice_permanently', true );
			} else if (
				target.closest( '.patterns-construction-rn-temporary-close' )
			) {
				this.modifySettings(
				...
				const installBtn = target.closest(
					'.patterns-construction-install-plugins'
				);
				if ( installBtn ) {
					installBtn.classList.add(
						'patterns-construction-processing'
					);
					this.installPlugins();
				}
			}
	...
	updateUI() {
		...
		if ( ! this.isLoadReview() ) {
			const reviewNoticeElement = document.getElementById(
				'patterns-construction-review-notice'
			);
